...
import {
  ...
  nameSchema,
  subCategorySchema,
} from "../../domain/validation/schemas/Product";
...
export class ManagerProduct {
  ...
  updateCamp(data: any, uptype: OperetionsType) {
    const operations = {
      name: async (data: any) => {
        if (
          !(await new ValidationData().execute(data.id, IdSchema)) ||
          !(await new ValidationData().execute(data.name, nameSchema))
        ) {
      ...
      description: async (data: any) => {
        if (
          !(await new ValidationData().execute(data.id, IdSchema)) ||
          !(await new ValidationData().execute(
            data.description,
            descriptionSchema
          ))
        ) {
      ...
      category: async (data: any) => {
        if (
          !(await new ValidationData().execute(data.id, IdSchema)) ||
          !(await new ValidationData().execute(data.category, categorySchema))
        ) {
          return new Result<Error, string>(
            new Error(),
            "",
      ...
      stock: async (data: updateStockType) =>{
     ...
     /**  price: async (data: any) => {
        if (
          !(await new ValidationData().execute(data.id, IdSchema)) ||
          !(await new ValidationData().execute(data.price))
        ) {
          return new Result<Error, string>(
            new Error(),
            "",
      ...
      priceCurrency: async (data: any) => {
        if (
          !(await new ValidationData().execute(data.id, IdSchema)) ||
          !(await new ValidationData().execute(data.description))
        ) {
          return new Result<Error, string>(
            new Error(),
            "",
    ...
    function updateProductData(data: any, operation: OperetionsType) {
      return (
        ...
        new Result<Error, string>(
          new Error(),
          "",
          "Cannot Change this Camp"
        ).fail()
      );
    }
  ...
  async getProduct(id: string) {
    ...
    return new Result<Error, ReturnGetProductType>(
      new Error(),
      product,
      "Validation Error, Wrong Data!"
    ).success();
